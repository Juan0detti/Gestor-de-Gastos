...
export default function Ahorros() {
  ...
  const [saldo, setSaldo] = useState(getBalance());
  ...
  // Al montar el componente, carga los ahorros desde almacenamiento
  useEffect(() => {
    const storedAhorros = getSaves();
    setAhorros(storedAhorros);
  }, []);
...
*/
  ...
  const handleEdicion = () => {
    ...
    try {
      ...
      if (
        isNaN(nuevosDatosTransformados.monto_actual) ||
        nuevosDatosTransformados.monto_actual < 0
      ) {
        throw new Error("El monto actual no puede ser negativo.");
      }

      if (
        nuevosDatosTransformados.monto_actual >
        nuevosDatosTransformados.monto_objetivo
      ) {
      ...
      if (nuevosDatosTransformados.monto_actual > balance.montoLibre) {
        throw new Error(
          "El monto actual no puede superar el saldo disponible."
        );
      }
      ...
      setValidationErrors({
        ...
        monto_objetivo: error.message.includes("monto objetivo")
          ? error.message
          : "",
        fecha_fin: error.message.includes("fecha") ? error.message : "",
        monto_actual:
          error.message.includes("monto actual") ||
        ...
        {ahorros.length === 0 ? (
          ...
          ahorros.map((ahorro) => {
            const porcentaje = Math.min(
              100,
              (ahorro.monto_actual / ahorro.monto_objetivo) * 100
            );

            return (
              <div
                key={ahorro.id}
                className="ahorro-card"
              ...
              >
                ...
                >
                  ...
                  <span>
                    ${ahorro.monto_actual.toFixed(2)}/$
                    {ahorro.monto_objetivo.toFixed(2)}
                  </span>
                </div>
        ...
        <form className="modal" onSubmit={handleRegistro}>
          <div className="modalContenido">
            ...
            {!!formData.monto_objetivo && (
              <>
                <label>Asignar un monto inicial:</label>
                ...
                <div className="range-wrapper">
                  <div className="range-labels">
                    <span
                      ...
                      style={{
                        left: `${formData.porcentaje || 0}%`,
                      }}
                    >
                      $
                      {saldo.getMontoLibre() >= formData.monto_objetivo
                        ? Math.round(
                            (formData.porcentaje / 100) *
                              formData.monto_objetivo
                          ).toLocaleString()
                        : Math.round(
                            (formData.porcentaje / 100) * saldo.getMontoLibre()
                          ).toLocaleString()}
                    </span>
                  </div>
                  ...
                  <div className="range-container">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const maximoAsignable = Math.min(
                          saldo.getMontoLibre(),
                          formData.monto_objetivo
                        );
                        const nuevoMonto =
                          (nuevoPorcentaje / 100) * maximoAsignable;

                        setFormData((prev) => ({
                          ...prev,
                          porcentaje: nuevoPorcentaje,
                          monto_actual: Math.round(nuevoMonto),
                        }));
                      }}
                    />
                  </div>

                  {/* Debajo de la barra, monto máximo y restante */}
                  <div
                  ...
                  >
                    <span style={{ fontSize: "0.85em" }}>
                      Máximo: $
                      {saldo.getMontoLibre() >= formData.monto_objetivo
                        ? Number(formData.monto_objetivo).toLocaleString()
                        : saldo.getMontoLibre().toLocaleString()}
                    </span>
                    {saldo.getMontoLibre() < formData.monto_objetivo && (
                      <span style={{ color: "red", fontSize: "0.85em" }}>
                        Falta: $
                        {(
                          formData.monto_objetivo - saldo.getMontoLibre()
                        ).toLocaleString()}
                      </span>
                    )}
                  </div>
